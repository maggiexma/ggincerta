...
Regular shape glyphs can be treated as ordinary point-like symbols and
used together with
[`bivariate_scale()`](https://maggiexma.github.io/ggincerta/reference/bivariate_scale.md).
Drop-shaped glyphs use rotation angle to represent uncertainty. Chernoff
glyphs are adapted from the `ggChernoff` package and work with the
`smile` aesthetic.
