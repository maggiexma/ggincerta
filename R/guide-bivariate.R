...
GuideBivariate <- ggproto(
  ...
  resolve_guide_info = function(self, scale, key) {
    attr(key, "bivariate_guide_info") %||%
      self$get_bivariate_guide_info(scale)
  },

  resolve_title = function(self, title, scale) {
    scale_name <- scale$name

    if (!is_waiver(scale_name) &&
        !is.null(scale_name) &&
        scale_name != "")
      return(scale_name)
  ...
  measure_labels = function(self,
    ...
    measure_fn <- if (what == "width")
      grid::grobWidth
    else
      grid::grobHeight

    grobs <- lapply(as.character(labels), function(lbl)
      self$make_text_grob(element, lbl))

    do.call(grid::unit.pmax, lapply(grobs, measure_fn))
  },

  get_var1_label_positions = function(self, labels, discrete = FALSE) {
    if (discrete) {
      (seq_along(labels) - 0.5) /
        length(labels)
    } else {
      seq(0, 1, length.out = length(labels))
    }
  },

  get_var2_label_positions = function(self, labels, discrete = FALSE) {
    if (discrete) {
      (seq_along(labels) - 0.5) /
        length(labels)
    } else {
  ...
  compute_bivariate_layout = function(self, params, elements) {
    ...
    if (!is.null(params$var2_labels)) {
      left_anno_w <- left_anno_w +
        var2_label_w +
        label_gap
    }

    if (!is.null(params$var2_title)) {
      left_anno_w <- left_anno_w +
        var2_title_w +
        title_gap
    }
  ...
  compute_panel_geometry = function(self, layout) {
    ...
    to_npc_x <- function(u)
      grid::unit(grid::convertWidth(u, "mm", valueOnly = TRUE) / decor_w_mm,
                 "npc")

    to_npc_y <- function(u)
      grid::unit(grid::convertHeight(u, "mm", valueOnly = TRUE) / decor_h_mm,
                 "npc")
  ...
  build_var1_label_grobs = function(self, labels, panel, layout, params = NULL) {
    ...
    xs <- if (isTRUE(params$x_discrete)) {
      (seq_along(labels) - 0.5) / length(labels)
    } else {
      seq(0, 1, length.out = length(labels))
    }
    ...
    if (rotated) {
      y <- panel$to_npc_y(layout$bottom_anno_h -
                            layout$label_gap -
                            0.5 * layout$var1_label_w)
  ...
  build_var2_label_grobs = function(self, labels, panel, layout, params = NULL) {
    if (is.null(labels))
      return(list())

    ys <- self$get_var2_label_positions(labels, discrete = isTRUE(params$y_discrete))

    x <- panel$to_npc_x(layout$left_anno_w -
                          layout$label_gap)
  ...
  build_var2_title_grob = function(self, title, labels, panel, layout, params = NULL) {
    ...
    list(
      self$make_text_grob(
        ...
        hjust = 0.5,
        vjust = 0.5,
        angle = if (isTRUE(params$rotated))
          270
        else-90
      )
    )
  ...
  extract_params = function(self, scale, params, title = NULL, ...) {
    guide_info <-
      self$resolve_guide_info(scale, params$key)

    if (is.null(guide_info))
      return(NULL)

    params$title <-
      self$resolve_title(title, scale)
    ...
    params$var1_labels <-
      guide_info$x_labels %||%
      format(guide_info$x_breaks)

    params$var2_labels <-
      guide_info$y_labels %||%
      format(guide_info$y_breaks)

    params$var1_title <-
      guide_info$var1_title

    params$var2_title <-
      guide_info$var2_title

    params$x_discrete <-
      isTRUE(guide_info$x_discrete)

    params$y_discrete <-
      isTRUE(guide_info$y_discrete)

    params$aesthetics <-
      guide_info$aesthetics
  ...
  setup_params = function(self, params) {
    ...
    if (is.null(params$aesthetics) &&
        !is.null(params$key)) {
      params$aesthetics <-
        intersect(names(params$key), c("fill", "colour", "color"))
    }
  ...
  build_decor = function(self, decor, grobs, elements, params) {
    if (is.null(params$key) ||
        is.null(params$bivar_n_breaks))
      return(zeroGrob())

    fill_mat <- self$build_fill_matrix(params$key[[params$aesthetics[1]]], params$bivar_n_breaks)

    layout_params <- params[c(
      "var1_labels",
      "var2_labels",
      "var1_title",
      "var2_title",
    ...
    layout <-
      self$compute_bivariate_layout(layout_params, elements)

    panel <-
      self$compute_panel_geometry(layout)
    ...
    grob_list <- c(
      ...
      self$build_var2_label_grobs(params$var2_labels, panel, layout, params),
  ...
  measure_grobs = function(self, grobs, params, elements) {
    layout <-
      self$compute_bivariate_layout(params, elements)

    list(
      title_w =
        grid::grobWidth(grobs$title),
  ...
  arrange_layout = function(self, key, sizes, params, elements) {
    list(
      has_title =
        !inherits(sizes$title_h, "zeroUnit"),
      title_position = "top"
    )
  },
  ...
  assemble_drawing = function(self,
    ...
    pad_t <-
      grid::unit(mg[[1]], mg_unit)

    pad_r <-
      grid::unit(mg[[2]], mg_unit)

    pad_b <-
      grid::unit(mg[[3]], mg_unit)

    pad_l <-
      grid::unit(mg[[4]], mg_unit)

    has_title <-
      !inherits(grobs$title, "zeroGrob")
    ...
    if (isTRUE(params$rotated)) {
      ...
      top_overhang <-
        grid::unit.pmax(grid::unit(0, "mm"), 0.5 *
                          (rot_h -
                             sizes$key_h))
    }
